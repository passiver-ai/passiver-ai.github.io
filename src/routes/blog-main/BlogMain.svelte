...
<svelte:head>
  ...
  <meta name="description" content="{data.blogName}" />
...
<!--      Boost your sales through SEO-optimized blog.-->
<!--      hello@inblog.ai You can change this text in Blog settings-->
    </p>
  </header>
    ...
    <div class="md:border-l md:border-zinc-100 md:pl-6 md:dark:border-zinc-700/40">
      <div class="flex max-w-3xl flex-col space-y-16">
        ...
        {#each data.post as post, index}
          <article class="md:grid md:grid-cols-4 md:gap-4 relative">
            <div>
              <div class="bg-cover bg-center h-32 relative z-10" style="background-image: url('{post.firstImageUrl}')"></div>
            </div>
            <div class="md:col-span-3 group flex flex-col items-start">
              <time class="relative z-10 order-first mb-1 items-center text-sm text-zinc-400 dark:text-zinc-500" datetime="{post.updatedAt}">
                <Time timestamp="{post.updatedAt}" format="dddd @ h:mm A · MMMM D, YYYY"></Time>
              </time>
              <div class="text-base font-semibold tracking-tight text-zinc-800 dark:text-zinc-100">
                ...
                <a href="/@{post.blogName}/{post.slug}">
                  <span class="absolute -inset-x-4 -inset-y-6 z-20 sm:-inset-x-6 sm:rounded-2xl"></span>
                  <h2 class="relative z-10">{post.title}</h2>
                </a>
              </div>
              <div class="w-full overflow-hidden h-16">
